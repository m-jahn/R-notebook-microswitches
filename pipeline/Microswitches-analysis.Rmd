...
Import an annotation file that matches regular AA postions to Ballasrteros-Weinstein annotation for GPCR receptors.
...
df_summary <- df_combined %>%
  group_by(type, n_mutation, mutation, mutation_bw) %>%
...
```{r}
mat_summary <- df_summary %>% select(-sd_value, -mutation_bw) %>%
  pivot_wider(values_from = mean_value, names_from = type) %>%
...
format_labels <- function(lab) {unname(deframe(df_nom)[lab])}
