...
mat_heatmap <- mat_summary[order.dendrogram(as.dendrogram(mat_cluster)), c(1,2,4,3,5)]
```
...
gridExtra::grid.arrange(
  ...
  plot_cluster_dend + 
    theme(plot.margin = unit(c(0.25, 0.118, -0.3, 0.172),"npc")),
  plot_heatmap,
  nrow = 2
)
```
...
plot_pca_var <- data.frame(as.matrix(get_pca_var(pca_result)$contrib) *
  rep(get_eigenvalue(pca_result)$variance.percent/100, each = 5)) %>%
